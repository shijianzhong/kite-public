...
async function selectBatch(batch: BatchInfo) {
  ...
  try {
    ...
    console.log('Selecting batch:', { batchId: batch.id, latestBatchId: latestData.id, isLatestBatch });
...
>
  ...
  >
    ...
    {#if isSelectingBatch}
      <div class="absolute inset-0 bg-white/80 dark:bg-gray-800/80 rounded-lg flex items-center justify-center z-10">
        <div class="flex flex-col items-center gap-3">
          <div class="animate-spin h-8 w-8 border-3 border-gray-300 dark:border-gray-600 border-t-blue-500 dark:border-t-blue-400 rounded-full"></div>
    ...
    {#if !showBatchSelector}
      ...
      <div class="flex items-center justify-between mb-4">
        <button
          ...
          class="p-2 rounded-lg transition-colors {canNavigatePrevious() ? 'hover:bg-gray-100 dark:hover:bg-gray-700' : 'opacity-30 cursor-not-allowed'}"
        ...
        <button
          ...
          class="p-2 rounded-lg transition-colors {canNavigateNext() ? 'hover:bg-gray-100 dark:hover:bg-gray-700' : 'opacity-30 cursor-not-allowed'}"
        ...
        {#each calendarDays as date}
          ...
          <button
            ...
            class="relative p-2 h-10 rounded-lg transition-all
              {hasBatch && inRange ? 'hover:bg-blue-50 dark:hover:bg-blue-900/30 cursor-pointer' : 'cursor-default'}
              {today ? 'ring-2 ring-blue-500 dark:ring-blue-400' : ''}
              {!currentMonth || !inRange ? 'opacity-30' : ''}"
            aria-label="{date.getDate()} - {batchCount} batches"
          >
            <span class="text-sm {hasBatch && inRange ? 'font-medium text-gray-700 dark:text-gray-300' : 'text-gray-400 dark:text-gray-600'}">
              {date.getDate()}
            </span>
            ...
            {#if hasBatch && inRange}
              <div class="absolute bottom-0.5 left-1/2 transform -translate-x-1/2 flex gap-0.5">
                {#each Array(Math.min(batchCount, 3)) as _}
                  <div class="w-1 h-1 rounded-full bg-blue-500 dark:bg-blue-400"></div>
                {/each}
              </div>
            {/if}
          </button>
              ...
              <div class="flex justify-between items-center">
                <div class="flex-1">
                  <div class="flex items-center gap-2 mb-1">
                    ...
                    {#if index === 0}
                      <span class="text-xs bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-400 px-2 py-0.5 rounded">
