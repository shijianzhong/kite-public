...
import { IconX, IconPlus, IconCheck, IconInfoCircle, IconDownload, IconUpload } from '@tabler/icons-svelte';
...
const localizedPresets = $derived(
	contentFilter.presets.map(preset => {
		...
		const label = translatedLabel !== `settings.contentFilter.preset.${preset.id}` 
			? translatedLabel 
			: preset.label;
...
<div class="space-y-6">
	...
	<div>
		...
		<div class="grid grid-cols-2 gap-2">
			{#each localizedPresets as preset}
				<div class="relative">
					<button
						onclick={() => contentFilter.togglePreset(preset.id, dataLanguage.current)}
						...
						<span class="flex items-center gap-2">
							...
							{#if preset.tooltip}
								<Tooltip text={preset.tooltip} position="top">
									<IconInfoCircle size={14} class="text-gray-400 dark:text-gray-500" />
								</Tooltip>
							{/if}
						</span>
	...
	{#if contentFilter.keywords.length > 0}
		<div>
			...
			{#each contentFilter.activePresets as presetId}
				...
				{#if preset}
					<div class="mb-3">
						<div class="text-xs text-gray-500 dark:text-gray-400 mb-1">{preset.label}:</div>
				...
				<div class="mb-3">
					...
					<div class="flex flex-wrap gap-2">
						{#each customKeywords as keyword}
							<span class="inline-flex items-center gap-1 rounded-full bg-gray-100 px-3 py-1 text-sm text-gray-700 dark:bg-gray-700 dark:text-gray-300">
								{keyword}
								<button
	...
	<div class="space-y-2">
		<Select
			...
			onChange={(value: string) => contentFilter.setFilterScope(value as 'title' | 'summary' | 'all')}
		/>
	...
	<div class="flex items-center justify-between">
		...
		<label class="relative inline-flex cursor-pointer items-center">
			<input
				...
				onchange={(e) => contentFilter.setShowFilteredCount(e.currentTarget.checked)}
			/>
			<div class="peer h-6 w-11 rounded-full bg-gray-200 after:absolute after:left-[2px] after:top-0.5 after:h-5 after:w-5 after:rounded-full after:border after:border-gray-300 after:bg-white after:transition-all after:content-[''] peer-checked:bg-blue-600 peer-checked:after:translate-x-full peer-checked:after:border-white dark:bg-gray-700 dark:after:bg-gray-600 dark:peer-checked:bg-blue-500"></div>
		</label>
	</div>
