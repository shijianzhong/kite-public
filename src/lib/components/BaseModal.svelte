...
const modalTransition = position === 'bottom' 
	? { y: 200, duration: 200 }
	: position === 'top'
	? { y: -50, duration: 200 }
	: { duration: 200 };
...
{#if isOpen}
	...
	<div
		class="fixed inset-0 bg-black/50 flex {positionClasses[position]}"
	...
	>
		...
		<div
			...
			class="relative w-full {sizeClasses[size]} {position === 'bottom' ? 'rounded-t-2xl' : 'rounded-lg'} bg-white shadow-xl dark:bg-gray-800 {className}"
			role="dialog"
			aria-modal="true"
