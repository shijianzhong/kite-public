...
export function buildCitationMapping(
  ...
  const findArticle = (
    ...
    if (!searchDomain.includes(".")) {
      ...
      if (exactBaseMatches.length > 0) {
        ...
        const uniqueDomains = [
          ...new Set(exactBaseMatches.map((a) => a.domain)),
        ];
        ...
        if (uniqueDomains.length === 1) {
          ...
          console.warn(
            "Removing this citation to avoid showing incorrect information",
          );
          return undefined;
        }
      }
    ...
    for (const citation of citations) {
      ...
      if (!article) {
        ...
        if (matchingDomains.length === 0) {
          ...
          const articlesFromDomain = articles.filter((a) =>
            matchingDomains.includes(a.domain),
          );
          console.warn(
            `Articles available from matching domain(s): ${articlesFromDomain.length}`,
          );
        }
        continue;
      }
  ...
  if (story.technical_details?.length) {
    story.technical_details.forEach((detail: string) =>
  ...
  if (story.scientific_significance?.length) {
    story.scientific_significance.forEach((point: string) =>
      processCitations(point),
    );
  }
  ...
  if (story.performance_statistics?.length) {
    story.performance_statistics.forEach((stat: string) =>
      processCitations(stat),
    );
  }

  if (story.user_experience_impact?.length) {
    story.user_experience_impact.forEach((impact: string) =>
      processCitations(impact),
    );
  }

  if (story.gameplay_mechanics?.length) {
    story.gameplay_mechanics.forEach((mechanic: string) =>
      processCitations(mechanic),
    );
  }

  if (story.gaming_industry_impact?.length) {
    story.gaming_industry_impact.forEach((impact: string) =>
      processCitations(impact),
    );
  }
 ...
 */
export function replaceWithNumberedCitations(
  text: string,
